	...
	public void imprime(){
		...
		for(int i=0; i<objeto.length; i++){
			for(int j=0; j<objeto[0].length; j++){
				if(objeto[i][j]!=null){
					if(objeto[i][j] instanceof Livre){
						Livre teste1 = (Livre) objeto[i][j];
						valor = teste1.getValor();
						System.out.print(valor + " ");
					}else if(objeto[i][j] instanceof Parede){
						Parede teste2 = (Parede) objeto[i][j];
						valor = teste2.getValor();
						System.out.print(valor + " ");
